CREATE TABLE FIDE_ESTADO_TB (
    ID_ESTADO NUMBER(1), -- 1 = Activo, 0 = Inactivo
    DESCRIPCION VARCHAR2(50) NOT NULL,
    CONSTRAINT FIDE_ESTADO_ID_ESTADO_PK PRIMARY KEY (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de País
CREATE TABLE FIDE_PAIS_TB (
    ID_PAIS NUMBER PRIMARY KEY,
    PAIS VARCHAR2(50) NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Provincia
CREATE TABLE FIDE_PROVINCIA_TB (
    ID_PROVINCIA NUMBER PRIMARY KEY,
    PROVINCIA VARCHAR2(50) NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Cantón
CREATE TABLE FIDE_CANTON_TB (
    ID_CANTON NUMBER PRIMARY KEY,
    CANTON VARCHAR2(50) NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Distrito
CREATE TABLE FIDE_DISTRITO_TB (
    ID_DISTRITO NUMBER PRIMARY KEY,
    DISTRITO VARCHAR2(50) NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Dirección
CREATE TABLE FIDE_DIRECCION_TB (
    ID_DIRECCION NUMBER PRIMARY KEY,
    CALLE VARCHAR2(100),
    NUMERO_CASA VARCHAR2(10),
    OTRAS_SENAS VARCHAR2(100),
    ID_PAIS NUMBER,
    ID_PROVINCIA NUMBER,
    ID_CANTON NUMBER,
    ID_DISTRITO NUMBER,
    ID_ESTADO NUMBER(1),
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_PAIS) REFERENCES FIDE_PAIS_TB(ID_PAIS),
    FOREIGN KEY (ID_PROVINCIA) REFERENCES FIDE_PROVINCIA_TB(ID_PROVINCIA),    
    FOREIGN KEY (ID_CANTON) REFERENCES FIDE_CANTON_TB(ID_CANTON),
    FOREIGN KEY (ID_DISTRITO) REFERENCES FIDE_DISTRITO_TB(ID_DISTRITO),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Usuarios
CREATE TABLE FIDE_USUARIO_TB (
    ID_USUARIO NUMBER PRIMARY KEY,
    NOMBRE VARCHAR2(50) NOT NULL,
    PRIMER_APELLIDO VARCHAR2(50) NOT NULL,
    SEGUNDO_APELLIDO VARCHAR2(50) NOT NULL,
    CORREO VARCHAR2(100) UNIQUE NOT NULL,
    TELEFONO VARCHAR2(20),
    ID_ESTADO NUMBER(1) NOT NULL,
    ID_DIRECCION NUMBER,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO),
    FOREIGN KEY (ID_DIRECCION) REFERENCES FIDE_DIRECCION_TB(ID_DIRECCION)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Producto
CREATE TABLE FIDE_PRODUCTO_TB (
    ID_PRODUCTO NUMBER PRIMARY KEY,
    NOMBRE VARCHAR2(100) NOT NULL,
    DESCRIPCION VARCHAR2(500),
    PRECIO NUMBER(10, 2) NOT NULL,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Promoción
CREATE TABLE FIDE_PROMOCION_TB (
    ID_PROMOCION NUMBER PRIMARY KEY,
    NOMBRE VARCHAR2(100) NOT NULL,
    DESCUENTO_PORCENTAJE NUMBER(5, 2),
    FECHA_INICIO DATE,
    FECHA_FIN DATE,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Promoción de Producto
CREATE TABLE FIDE_PRODUCTO_PROMOCION_TB (
    ID_PROMOCION NUMBER NOT NULL,
    ID_PRODUCTO NUMBER NOT NULL,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    PRIMARY KEY (ID_PROMOCION, ID_PRODUCTO),
    FOREIGN KEY (ID_PROMOCION) REFERENCES FIDE_PROMOCION_TB (ID_PROMOCION),
    FOREIGN KEY (ID_PRODUCTO) REFERENCES FIDE_PRODUCTO_TB (ID_PRODUCTO),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Inventario
CREATE TABLE FIDE_INVENTARIO_TB (
    ID_INVENTARIO NUMBER PRIMARY KEY,
    ID_PRODUCTO NUMBER NOT NULL,
    CANTIDAD NUMBER NOT NULL,
    FECHA_ULTIMA_ACTUALIZACION DATE DEFAULT SYSDATE,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_PRODUCTO) REFERENCES FIDE_PRODUCTO_TB (ID_PRODUCTO),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Proveedor
CREATE TABLE FIDE_PROVEEDOR_TB (
    ID_PROVEEDOR NUMBER PRIMARY KEY,
    NOMBRE VARCHAR2(100) NOT NULL,
    TELEFONO VARCHAR2(15),
    EMAIL VARCHAR2(100) NOT NULL,
    ID_DIRECCION NUMBER, 
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO),
    FOREIGN KEY (ID_DIRECCION) REFERENCES FIDE_DIRECCION_TB(ID_DIRECCION)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Producto Proveedor
CREATE TABLE FIDE_PRODUCTO_PROVEEDOR_TB (
    ID_PRODUCTO NUMBER NOT NULL,
    ID_PROVEEDOR NUMBER NOT NULL,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    PRIMARY KEY (ID_PRODUCTO, ID_PROVEEDOR),
    FOREIGN KEY (ID_PRODUCTO) REFERENCES FIDE_PRODUCTO_TB (ID_PRODUCTO),
    FOREIGN KEY (ID_PROVEEDOR) REFERENCES FIDE_PROVEEDOR_TB (ID_PROVEEDOR),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Carrito (Temporal para guardar productos antes de hacer un pedido)
CREATE TABLE FIDE_CARRITO_TB (
    ID_CARRITO NUMBER PRIMARY KEY,
    ID_USUARIO NUMBER NOT NULL,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_USUARIO) REFERENCES FIDE_USUARIO_TB (ID_USUARIO),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Detalle Carrito (Productos dentro del Carrito)
CREATE TABLE FIDE_DETALLE_CARRITO_TB (
    ID_DETALLE_CARRITO NUMBER PRIMARY KEY,
    ID_CARRITO NUMBER NOT NULL,
    ID_PRODUCTO NUMBER NOT NULL,
    CANTIDAD NUMBER NOT NULL,
    SUBTOTAL NUMBER(10, 2),
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_CARRITO) REFERENCES FIDE_CARRITO_TB (ID_CARRITO),
    FOREIGN KEY (ID_PRODUCTO) REFERENCES FIDE_PRODUCTO_TB (ID_PRODUCTO),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Método Pago
CREATE TABLE FIDE_METODO_PAGO_TB (
    ID_METODO_PAGO NUMBER PRIMARY KEY,
    NOMBRE VARCHAR2(50) NOT NULL,
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Local
CREATE TABLE FIDE_LOCAL_TB (
    ID_LOCAL NUMBER PRIMARY KEY,
    NOMBRE VARCHAR2(100) NOT NULL,
    ID_DIRECCION NUMBER,
    TELEFONO VARCHAR2(15),
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO),
    FOREIGN KEY (ID_DIRECCION) REFERENCES FIDE_DIRECCION_TB(ID_DIRECCION)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Pedido (Generado al confirmar el Carrito)
CREATE TABLE FIDE_PEDIDO_TB (
    ID_PEDIDO NUMBER PRIMARY KEY,
    ID_USUARIO NUMBER NOT NULL,
    ID_METODO_PAGO NUMBER NOT NULL,
    FECHA_PEDIDO DATE DEFAULT SYSDATE,
    TOTAL NUMBER(10, 2) NOT NULL,
    ID_ESTADO NUMBER NOT NULL,
    ID_LOCAL NUMBER NOT NULL, 
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_USUARIO) REFERENCES FIDE_USUARIO_TB (ID_USUARIO),
    FOREIGN KEY (ID_METODO_PAGO) REFERENCES FIDE_METODO_PAGO_TB (ID_METODO_PAGO),
    FOREIGN KEY (ID_LOCAL) REFERENCES FIDE_LOCAL_TB (ID_LOCAL),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;

-- Tabla de Detalle Pedido (Productos dentro del Pedido)
CREATE TABLE FIDE_DETALLE_PEDIDO_TB (
    ID_DETALLE_PEDIDO NUMBER PRIMARY KEY,
    ID_PEDIDO NUMBER NOT NULL,
    ID_PRODUCTO NUMBER NOT NULL,
    CANTIDAD NUMBER NOT NULL,
    SUBTOTAL NUMBER(10, 2),
    TOTAL NUMBER(10, 2),
    ID_ESTADO NUMBER NOT NULL,
    CREADO_POR VARCHAR2(100),
    MODIFICADO_POR VARCHAR2(100),
    FECHA_CREACION TIMESTAMP,
    FECHA_MODIFICACION TIMESTAMP,
    ACCION VARCHAR2(100), 
    FOREIGN KEY (ID_PEDIDO) REFERENCES FIDE_PEDIDO_TB (ID_PEDIDO),
    FOREIGN KEY (ID_PRODUCTO) REFERENCES FIDE_PRODUCTO_TB (ID_PRODUCTO),
    FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB (ID_ESTADO)
) TABLESPACE FIDE_PROYECTOFINAL_TBS;